...
write.csv(comments_in_keyword_threads, file.path(comments_directory, "aids_related_comments.csv"), row.names = FALSE)
...
# Merge thread relevancy scores back to thread-level dataframe
threads_summary_df <- comments_in_keyword_threads %>%
  distinct(Thread.ID, newsgroup) %>%
  left_join(thread_relevancy_scores, by = "Thread.ID")

# Save the threads dataframe to CSV
write.csv(threads_summary_df, file.path(threads_directory, "aids_related_threads.csv"), row.names = FALSE)
```
...
tab_df(keywords_summary_df, file = paste0(output_directory, "images and tables/aids_descriptive_statistics_table.html"))
```