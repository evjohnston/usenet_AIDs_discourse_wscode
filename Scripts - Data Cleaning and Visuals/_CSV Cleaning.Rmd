...
```{r}
rm(list = ls()) 
setwd("/Users/emerson/Github/usenet_early_histories_project")
```
...
# Load the datasets
all_threads <- read.csv("CSV Files/Threads/combined_threads.csv")
all_comments <- read.csv("CSV Files/Comments/combined_comments.csv")
...
tab_df(summary_df, file = "Images and Tables/descriptive_statistics_table.html")
```
...
# Create the table
summary_table <- kable(summary_df, format = "html", table.attr = "style='width:100%;'") %>%
  kable_styling(full_width = FALSE) %>%
  row_spec(0, bold = TRUE) %>%  # Make the first row bold
  row_spec(nrow(summary_df), hline_after = TRUE)  # Add a line after the second to last row
