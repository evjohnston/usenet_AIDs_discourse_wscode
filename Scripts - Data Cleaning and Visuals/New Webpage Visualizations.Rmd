...
```{r Interactive Co-occurrence Network with Plotly, eval=TRUE, include=TRUE}
# Load necessary library
library("threejs")

# Prepare data for 3D network
# We will use the filtered_co_occurrence and term_topic_df from the previous code block
...
# Create the 3D force-directed network
network_3d <- graphjs(
  graph_3d,
  vertex.size = log1p(term_frequency[filtered_terms]),
  vertex.label = V(graph_3d)$name,
  vertex.color = topic_colors,
  edge.width = E(graph_3d)$weight / max(E(graph_3d)$weight) * 5,  # Scale edge width
  main = "Interactive 3D Co-occurrence Network of Key Themes in AIDS Discussion"
)

# Save the interactive 3D network to an HTML file
htmlwidgets::saveWidget(network_3d, file = file.path(output_directory, "images and tables/new web visualizations", "interactive_plot_3d_cooccurrence_network.html"))
```
